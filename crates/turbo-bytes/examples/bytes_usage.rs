...
fn main() -> turbo_core::Result<()> {
    ...
    println!("ByteBuffer len: {}, capacity: {}", buf.len(), buf.capacity());

    // 2. Zero-Copy ByteView
    println!("\n--- 2. Zero-Copy ByteView ---");
    let view = ByteView::new(buf);
    let slice_view = view.slice(0..5)?;
    println!("Sliced view contents: {:?}", String::from_utf8_lossy(&slice_view));
    println!("Original view length: {}, slice length: {}", view.len(), slice_view.len());
    ...
    let pool = BufferPool::new();
    {
        ...
        println!("Pooled buffer len: {}, capacity: {}", pooled_buffer.len(), pooled_buffer.capacity());
    ...
    println!("Recycled buffer len (should be 0): {}", recycled_buffer.len());
