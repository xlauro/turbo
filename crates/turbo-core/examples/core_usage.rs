...
fn main() -> Result<()> {
    ...
    println!(
        ...
        tracking_alloc.allocated_bytes()
    );

    let ptr = tracking_alloc.alloc(layout)?;
    println!(
        "Allocated 1 packet. Current allocated bytes: {}",
        tracking_alloc.allocated_bytes()
    );
    ...
    unsafe {
        ...
        packet_ref.id = 1;
        packet_ref.val = core::f64::consts::PI;

        println!(
            "Packet contents: id={}, val={}",
            packet_ref.id, packet_ref.val
        );
    ...
    println!(
        "Deallocated. Current allocated bytes: {}",
        tracking_alloc.allocated_bytes()
    );
    ...
    let packet = DataPacket {
        id: 42,
        val: core::f64::consts::E,
    };
    println!("Packet stack size: {}", core::mem::size_of_val(&packet));
    println!(
        "Packet heap size (via EstimateSize): {}",
        packet.heap_size()
    );
...
fn check_packet_id(id: u64) -> Result<()> {
    ...
    turbo_ensure!(
        id > 0,
        Error::InvalidData {
            reason: "Packet ID must be non-zero"
        }
    );
    Ok(())
}
